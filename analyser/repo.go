...
func CloneRepo(url string) (string, string) {
	...
	if _, empty_err := os.Stat(repo_dir); os.IsNotExist(empty_err) {
		GeneralLog("Starting clone of repo: %s\n", url)
		...
		if head != nil {
			...
			if err != nil {
				WarningLog("CloneRepo, %s: An error occured when reading the commit log\n\terror: %v\n.", url, err)
			}
			...
			if err != nil {
				ExitLog(1, "CloneRepo,  %s: Error when iterating through commits.\n")
			}
		} else {
			WarningLog("CloneRepo,  %s: Repo was empty?\n")
		}
		...
		if err != nil {
			WarningLog("CloneRepo,  %s: Error when opening \"commits.csv\"\n\terror: %v\n", err)
		}
		defer f.Close()
